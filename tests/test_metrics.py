...
class TestMetrics:
    ...
    def test_metrics_isolation(self):
        ...
        assert INJECTIONS_TOTAL.labels(failure_type='cpu', status='skipped')._value.get() == 0
    ...
    def test_counter_increments(self):
        ...
        initial = INJECTIONS_TOTAL.labels(failure_type='cpu', status='success')._value.get()
        
        inject_cpu(config, dry_run=False)
        
        final = INJECTIONS_TOTAL.labels(failure_type='cpu', status='success')._value.get()
    ...
    def test_dry_run_metrics(self):
        ...
        skipped = INJECTIONS_TOTAL.labels(failure_type='cpu', status='skipped')._value.get()
        assert skipped == 1