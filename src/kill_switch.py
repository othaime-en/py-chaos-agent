...
class KillSwitch:
    ...
    def start_monitoring(self, should_stop_agent_callback) -> None:
        ...
        self._monitor_thread = threading.Thread(
            target=self._monitor_loop, daemon=True, name="kill-switch-monitor"
        )
    ...
    def _should_trip_kill_switch(self) -> bool:
        ...
        if not is_healthy:
            ...
            logger.warning(
                f"Target health check failed "
                f"({self.consecutive_failures}/{self.config.consecutive_failures_threshold})",
                extra={
                    "consecutive_failures": self.consecutive_failures,
                    "active_injections": list(
                        self.failure_context.get_active_injections()
                    ),
                },
            )
...
def wrap_injection_with_context(
    failure_type: str, injection_func, failure_context: FailureContext, *args, **kwargs
):
    """
