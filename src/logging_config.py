...
class StructuredFormatter(logging.Formatter):
    ...
    def _format_text(self, record: logging.LogRecord) -> str:
        ...
        base_msg = (
            f"{timestamp} {record.levelname:8s} [{record.name}]"
            f"{corr_id_str} {record.getMessage()}"
        )
...
def setup_logging(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    log_format = os.environ.get("LOG_FORMAT") or get_config(
...
def log_failure_injection(
    logger: logging.Logger, failure_type: str, action: str, status: str, **kwargs
):
    """
...
def log_metric_event(
    logger: logging.Logger, metric_name: str, metric_value: Any, **kwargs
):
    """
    Log a metric event.
    ...
    logger.debug(
        ...
        extra={"metric_name": metric_name, "metric_value": metric_value, **kwargs},
    )
